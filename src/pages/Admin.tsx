      ...
      <header className="bg-white border-b border-slate-200 py-5 px-8 flex justify-between items-center sticky top-0 z-10 shadow-sm">
        <div className="flex items-center space-x-4">
          <Link to="/" className="font-display text-slate-900 font-extrabold text-2xl tracking-tight flex items-center">
            <span className="bg-emerald-600 text-white w-8 h-8 rounded-lg flex items-center justify-center mr-3 text-sm font-bold">BE</span>
            Bello El-Rufai
          </Link>
          <span className="text-slate-300">|</span>
          <span className="text-slate-600 font-medium flex items-center font-display">
            <FolderPlus className="w-5 h-5 mr-2 text-emerald-600" />
            Admin Dashboard
          </span>
        </div>
        <Link to="/" className="flex items-center text-sm font-bold text-slate-500 hover:text-emerald-700 hover:bg-emerald-50 px-4 py-2 rounded-lg transition-colors font-display">
            ...
            <form onSubmit={handleSubmit} className="space-y-6">
              <div>
                ...
                <input 
                  ...
                  className="w-full px-4 py-3 bg-slate-50 border border-slate-200 rounded-xl focus:ring-2 focus:ring-emerald-500 focus:border-emerald-500 focus:bg-white outline-none transition-all font-medium text-slate-900"
              ...
              <div>
                ...
                <textarea 
                  ...
                  className="w-full px-4 py-3 bg-slate-50 border border-slate-200 rounded-xl focus:ring-2 focus:ring-emerald-500 focus:border-emerald-500 focus:bg-white outline-none transition-all h-28 resize-none font-medium text-slate-900"
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <input 
                    ...
                    className="w-full px-4.5 py-3 bg-slate-50 border border-slate-200 rounded-xl focus:ring-2 focus:ring-emerald-500 focus:border-emerald-500 focus:bg-white outline-none transition-all font-medium text-slate-900 text-sm"
                  />
                </div>
                ...
                <div>
                  ...
                  <div className="flex bg-slate-100 p-1 rounded-xl">
                    <button
                      ...
                      className={`flex-1 py-2 rounded-lg text-xs font-bold transition-all flex items-center justify-center space-x-1 ${mediaType === 'image' ? 'bg-white text-emerald-700 shadow' : 'text-slate-500 hover:text-slate-900'}`}
                    >
                      <ImageIcon size={14} />
                      <span>Photo</span>
                    </button>
                    <button
                      ...
                      className={`flex-1 py-2 rounded-lg text-xs font-bold transition-all flex items-center justify-center space-x-1 ${mediaType === 'video' ? 'bg-white text-emerald-700 shadow' : 'text-slate-500 hover:text-slate-900'}`}
                    >
                      <Video size={14} />
                      <span>Video</span>
                    </button>
              ...
              <div>
                ...
                <div className="flex space-x-3">
                  <button
                    ...
                    className={`flex-1 py-3 px-4 rounded-xl border font-bold text-sm flex items-center justify-center space-x-2 transition-all ${mediaSource === 'upload' ? 'border-emerald-600 bg-emerald-50/50 text-emerald-700 shadow-sm' : 'border-slate-200 bg-slate-50 text-slate-600 hover:border-slate-300'}`}
                  >
                    <Upload size={16} />
                  ...
                  <button
                    ...
                    className={`flex-1 py-3 px-4 rounded-xl border font-bold text-sm flex items-center justify-center space-x-2 transition-all ${mediaSource === 'url' ? 'border-emerald-600 bg-emerald-50/50 text-emerald-700 shadow-sm' : 'border-slate-200 bg-slate-50 text-slate-600 hover:border-slate-300'}`}
                  >
                    <Globe size={16} />
                    <span>Web URL</span>
                  </button>
                      ...
                      {mediaFile ? (
                        ...
                        <div>
                          <p className="text-sm font-bold text-slate-800">
                            Drag & drop file here or <span className="text-emerald-700">browse</span>
                          </p>
                ...
                <div>
                  ...
                  <div className="relative flex items-center">
                    ...
                    <input 
                      ...
                      className="w-full pl-11 pr-4 py-3 bg-slate-50 border border-slate-200 rounded-xl focus:ring-2 focus:ring-emerald-500 focus:border-emerald-500 focus:bg-white outline-none transition-all font-medium text-slate-950 text-sm"
              ...
              {((mediaSource === 'upload' && mediaFile) || (mediaSource === 'url' && mediaUrlInput)) && (
                <div className="bg-slate-100 p-4.5 rounded-2xl border border-slate-200/60">
                  <div className="text-xs font-bold text-slate-500 uppercase tracking-wider mb-2 font-display flex items-center justify-between">
                    ...
                    <span className="text-[10px] bg-emerald-100 text-emerald-700 px-2 py-0.5 rounded font-black">Ready</span>
                  </div>
                    ...
                    {mediaSource === 'upload' && mediaFile ? (
                      mediaType === 'image' ? (
                        ...
                        <div className="w-full h-full flex flex-col items-center justify-center text-white p-4">
                          <Video size={28} className="text-emerald-500 mb-2 animate-pulse" />
              ...
              <button 
                ...
                className="w-full bg-gradient-to-r from-emerald-600 to-emerald-800 text-white font-bold text-lg py-4 rounded-xl hover:from-emerald-700 hover:to-emerald-900 transition-all disabled:from-slate-400 disabled:to-slate-500 shadow-lg hover:shadow-xl hover:-translate-y-0.5 flex justify-center items-center mt-4 font-display tracking-wide"
              >
                {isSubmitting ? (
                  ...
                  projects.map((project, index) => (
                    ...
                    >
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="text-xs font-bold text-emerald-700 mb-1">{new Date(project.date).toLocaleDateString('en-US', { month: 'long', day: 'numeric', year: 'numeric' })}</div>
                        <h4 className="font-display text-xl font-bold text-slate-900 mb-2 truncate group-hover:text-emerald-700 transition-colors">{project.title}</h4>
